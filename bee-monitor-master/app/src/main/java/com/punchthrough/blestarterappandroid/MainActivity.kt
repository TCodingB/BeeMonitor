...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startBleScan()
        Toast.makeText(this@MainActivity, "Iskanje naprav", Toast.LENGTH_SHORT).show()
    ...
    private fun startBleScan() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && !isLocationPermissionGranted) {
            ...
            /*
            //Filtritanje naprav po MAC
            val filters:List<ScanFilter>
            filters = ArrayList()
            val address = "7F:13:B9:50:B1:88"
            ...
            filters = ArrayList()
            val ime = "BeeHive"
            val filter = ScanFilter.Builder().setDeviceName(ime
            ).build()
            filters.add(filter)
...
private fun BluetoothLeScanner.startScan(filter: ScanFilter?, scanSettings: ScanSettings?, scanCallback: ScanCallback) {
